...
function onSubmit(values) {
  const data = {data:{type:"CREATE",table:"moltLimits",authorId:1},datatable:{...values,speciesId:1}}
  console.log(values)
      ...
      <Container>
        {data?         <form onSubmit={handleSubmit(onSubmit)}>
          <FormControl isInvalid={errors.type}>
            ...
            <Select
              {...register("moltLimitId", { valueAsNumber:true,required: "Necessário" })}
              placeholder="Selecione"
            >

{              data.map((ml,i)=>
              <option key={i} value={+ml.id}>{ml.pt_name}</option>)
